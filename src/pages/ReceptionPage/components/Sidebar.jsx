...
import {
  ...
  ChevronLeft, ChevronRight, Sun, Moon, X, ClipboardList
} from "lucide-react";
    ...
    >
      ...
      <div className="flex items-center justify-between px-4 py-5"
        ...
        {!isCollapsed && (
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <p className="text-sm font-extrabold tracking-tight" style={{ color: "var(--text)" }}>MedReception</p>
              <p className="text-[10px] font-medium" style={{ color: "var(--text)", opacity: .4 }}>Reception Desk</p>
            </div>
          </div>
        )}
      ...
      <nav className="flex-1 py-4 px-2 flex flex-col gap-0.5 overflow-y-auto">
        ...
        {mainNav.map(({ label, icon: Icon, path }) => (
          <NavLink key={label} to={path} end={path === "/reception"} onClick={handleNavClick}
        ...
        {!isCollapsed ? (
          <div className="flex items-center gap-3 px-3 py-2.5 rounded-xl"
            ...
            <div className="min-w-0">
              <p className="text-xs font-bold truncate" style={{ color: "var(--text)" }}>Receptionist</p>
              <p className="text-[10px] truncate" style={{ color: "var(--text)", opacity: .4 }}>reception@medlab.com</p>
            </div>
          </div>
        ) : (
