...
const DoctorSchedule = () => {
  ...
  // Track previous appointment IDs to detect new ones
  const prevIdsRef = useRef(new Set());

  const fetchAppointments = () => dispatch(fetchDoctorAppointments());

  useEffect(() => {
    fetchAppointments();

    // Poll every 30s for new appointment requests
    const interval = setInterval(async () => {
      const result = await dispatch(fetchDoctorAppointments());
      if (fetchDoctorAppointments.fulfilled.match(result)) {
        const freshList = result.payload;
        freshList.forEach((appt) => {
          if (
            !prevIdsRef.current.has(appt._id) &&
            appt.status === "REQUESTED"
          ) {
            toast(
              `📋 New appointment request from ${appt.patientId?.name || "a patient"}`,
              {
                duration: 5000,
                style: {
                  background: "var(--card)",
        ...
        // Update tracked IDs
        prevIdsRef.current = new Set(freshList.map((a) => a._id));
      }
    }, 30000);
  ...
  // Initialize prevIdsRef once data loads
  useEffect(() => {
    if (appointments.length && prevIdsRef.current.size === 0) {
      prevIdsRef.current = new Set(appointments.map((a) => a._id));
    }
  }, [appointments]);
  ...
  const statCards = [
    { label: "Total",    value: total,    icon: Calendar,    color: "#6a5acd", bg: "rgba(106,90,205,0.10)", border: "rgba(106,90,205,0.20)" },
    { label: "Accepted", value: accepted, icon: CheckCircle2, color: "#16a34a", bg: "rgba(34,197,94,0.10)",   border: "rgba(34,197,94,0.20)"   },
    { label: "Pending",  value: pending,  icon: Hourglass,   color: "#ca8a04", bg: "rgba(234,179,8,0.10)",   border: "rgba(234,179,8,0.20)"   },
    { label: "Rejected", value: rejected, icon: XCircle,     color: "#dc2626", bg: "rgba(239,68,68,0.10)",   border: "rgba(239,68,68,0.20)"   },
  ];
  ...
  return (
    <main className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h2 className="text-2xl font-bold" style={{ color: "var(--text)" }}>My Schedule</h2>
          <p className="text-sm mt-0.5" style={{ color: "var(--text)", opacity: 0.45 }}>
            {total} appointment{total !== 1 ? "s" : ""} in total
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {statCards.map(({ label, value, icon: Icon, color, bg, border }) => (
          <div key={label} className="rounded-2xl p-4 transition-shadow hover:shadow-md"
            style={{ background: "var(--card)", border: "1px solid var(--border)" }}>
            ...
            <p className="text-2xl font-extrabold" style={{ color: "var(--text)" }}>{value}</p>
            <p className="text-xs font-medium mt-0.5" style={{ color: "var(--text)", opacity: 0.45 }}>{label}</p>
          </div>
        ))}
      </div>
          ...
          return (
            <button key={f} onClick={() => setActiveFilter(f)}
              ...
              style={{
                background: isActive ? cfg.bg    : "var(--card)",
                color:      isActive ? cfg.color : "var(--text)",
                border:     isActive ? `1px solid ${cfg.color}40` : "1px solid var(--border)",
                opacity: isActive ? 1 : 0.6,
              }}>
              {filterLabel[f]}
              {f !== "All" && (
                <span className="ml-1.5 font-bold" style={{ opacity: isActive ? 1 : 0.5 }}>
                  {f === "REQUESTED" ? pending : f === "ACCEPTED" ? accepted : rejected}
                </span>
              )}
            </button>
      ...
      {loading ? (
        ...
        <div className="rounded-2xl p-12 flex flex-col items-center justify-center gap-3 text-center"
          ...
          <p className="text-sm font-semibold" style={{ color: "var(--text)" }}>No appointments found</p>
          <p className="text-xs" style={{ color: "var(--text)", opacity: 0.4 }}>
            {search ? "Try a different search term" : "Your schedule is clear for now"}
          </p>
        </div>
        ...
        <div className="rounded-2xl overflow-hidden"
          style={{ background: "var(--card)", border: "1px solid var(--border)" }}>
          <div className="px-5 py-3 grid grid-cols-[1fr_auto_auto] gap-4 items-center"
            style={{ borderBottom: "1px solid var(--border)", background: "rgba(106,90,205,0.04)" }}>
            <span className="text-[10px] font-bold uppercase tracking-widest"
              style={{ color: "var(--text)", opacity: 0.4 }}>Patient</span>
            <span className="text-[10px] font-bold uppercase tracking-widest"
              style={{ color: "var(--text)", opacity: 0.4 }}>Status</span>
